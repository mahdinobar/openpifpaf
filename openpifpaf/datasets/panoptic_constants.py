...
PANOPTIC_KINEMATIC_TREE_SKELETON = PANOPTIC_HAND_SKELETON
...
PANOPTIC_UPRIGHT_POSE = np.array([
    [0 ,0 , 2.0], # 'palm',  # 1
...
DENSER_PANOPTIC_HAND_CONNECTIONS = [
    c
    for c in DENSER_PANOPTIC_HAND_SKELETON
    if c not in PANOPTIC_HAND_SKELETON
]
...
def draw_skeletons(pose):
    ...
    ann = Annotation(keypoints=PANOPTIC_KEYPOINTS, skeleton=PANOPTIC_HAND_SKELETON)
    ann.set(pose, np.array(PANOPTIC_HAND_SIGMAS) * scale)
    draw_ann(ann, filename='/home/mahdi/HVR/git_repos/openpifpaf/docs/panoptic_references/skeleton_panoptic.png', keypoint_painter=keypoint_painter)

    ann = Annotation(keypoints=PANOPTIC_KEYPOINTS, skeleton=PANOPTIC_KINEMATIC_TREE_SKELETON)
    ann.set(pose, np.array(PANOPTIC_HAND_SIGMAS) * scale)
    draw_ann(ann, filename='/home/mahdi/HVR/git_repos/openpifpaf/docs/panoptic_references/skeleton_kinematic_tree_panoptic.png', keypoint_painter=keypoint_painter)

    ann = Annotation(keypoints=PANOPTIC_KEYPOINTS, skeleton=DENSER_PANOPTIC_HAND_SKELETON)
    ann.set(pose, np.array(PANOPTIC_HAND_SIGMAS) * scale)
    draw_ann(ann, filename='/home/mahdi/HVR/git_repos/openpifpaf/docs/panoptic_references/skeleton_dense_panoptic.png', keypoint_painter=keypoint_painter)


def print_associations():
    for j1, j2 in PANOPTIC_HAND_SKELETON:
        print(PANOPTIC_KEYPOINTS[j1 - 1], '-', PANOPTIC_KEYPOINTS[j2 - 1])
...
if __name__ == '__main__':
    ...
    draw_skeletons(PANOPTIC_UPRIGHT_POSE)
